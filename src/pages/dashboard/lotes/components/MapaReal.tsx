...
export const MapaReal = ({
  ...
  const centroMapa = useMemo(() => {
    ...
    if (totalPuntos === 0) {
      return { lat: 4.711, lng: -74.0721 } // Bogotá por defecto
    }
  ...
  useEffect(() => {
    ...
    trabajadoresData.forEach((trabajador, index) => {
      ...
      do {
        ...
        !puntoEnPoligono(workerLat, workerLng, primerLote.coordenadas) &&
          ...
          <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Preparando {lotes.length} lote{lotes.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        )}
